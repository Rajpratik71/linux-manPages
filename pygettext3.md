...
OPTIONS
       ...
       -x, --exclude-file=FILENAME
              ...
              extracted from the input files.  Each  string  to  be  excluded
              must appear on a line by itself in the file.

       -X, --no-docstrings=FILENAME
              Specify  a  file  that  contains a list of files (one per line)
              that should not have their docstrings extracted.  This is  only
              useful in conjunction with the -D option above.
...
BUGS
       ...
       switches with option arguments is broken, and in these  cases,  pyget‐
       text just defines additional switches.
