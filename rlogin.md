...
DESCRIPTION
     ...
     -Y      Enables trusted X11 forwarding.  Trusted X11 forwardings are not
             ...
             (Debian-specific: This option does nothing in the default con‐
             figuration: it is equivalent to “ForwardX11Trusted yes”, which
             is the default as described above.  Set the ForwardX11Trusted
             option to “no” to restore the upstream behaviour.  This may
             change in future depending on client-side improvements.)
...
AUTHENTICATION
     ...
     knows the public key, and only the user knows the private key.  ssh
     implements public key authentication protocol automatically, using one
     of the DSA, ECDSA, Ed25519 or RSA algorithms.  The HISTORY section of
     ...
     ~/.ssh/id_ed25519 (Ed25519), or ~/.ssh/id_rsa (RSA) and stores the pub‐
     lic key in ~/.ssh/id_dsa.pub (DSA), ~/.ssh/id_ecdsa.pub (ECDSA),
     ...
     vate keys.  See the CERTIFICATES section of ssh-keygen(1) for more
     information.
...
TCP FORWARDING
     ...
     service which is to be encrypted on the client machine, connecting to
     the same local port, and ssh will encrypt and forward the connection.
