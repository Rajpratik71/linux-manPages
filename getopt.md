...
DESCRIPTION
       ...
       parsed (parameters in the SYNOPSIS).  The second part  will  start  at
       the  first  non-option  parameter  that  is not an option argument, or
       ...
       parameter  is not an option (does not start with a '-', the first for‐
       mat in the SYNOPSIS), getopt will generate output that  is  compatible
       ...
       no  longer  compatible with other versions (the second or third format
       in the SYNOPSIS).  To  determine  whether  this  enhanced  version  of
...
OPTIONS
       ...
       -T, --test
              ...
              sion.   This  generates no output, and sets the error status to
       ...
       -V, --version
              Display version information and exit.  No other output is  gen‐
              erated.
...
OUTPUT
       ...
       ated as one parameter.  If the option has an argument, the next param‐
       eter will be the argument.  If the option takes an optional  argument,
...
SCANNING MODES
       ...
       collected at the end of output after a '--' parameter has been  gener‐
       ated.   Note  that this '--' parameter is still generated, but it will
       always be the last parameter in this mode.

COMPATIBILITY
       This version of getopt(1) is written to be as compatible  as  possible
       to  other  versions.  Usually you can just replace them with this ver‐
