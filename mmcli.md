...
DESCRIPTION
       ...
       ferent connection managers, like NetworkManager. Thanks to the  built-
       in  plugin architecture, ModemManager talks to very different kinds of
...
MANAGER OPTIONS
       -G, --set-logging=[ERR|WARN|INFO|DEBUG]
              ...
              vides  less  detail.  In most cases ERR (for displaying errors)
...
COMMON OPTIONS
       ...
       in.  They  can  be  found using mmcli -L. This produces something like
       (for modems only):
...
MODEM OPTIONS
       ...
       modes actually supported by the modem. In the perfect case,  the  fol‐
       lowing are possible:
       ...
       --factory-reset=CODE
              ...
              The CODE provided is vendor specific. Without the correct  ven‐
              dor  code,  it's  unlikely this operation will succeed. This is
       ...
       --create-bearer=['KEY1=VALUE1,KEY2=VALUE2,...']
              ...
              some VALUEs are listed below:
                ...
                'allowed-auth'
                       ...
                       lowedAuth                 value                  (e.g.
                       'none|pap|chap|mschap|mschapv2|eap').    Optional   in
                       3GPP.

                'user' User name (if any) required by the  network.  Optional
                       in 3GPP.
                ...
                'rm-protocol'
                       Protocol of the Rm interface, given  as  a  MMModemCd‐
                       maRmProtocol   value  (e.g.  'async',  'packet-relay',
                       'packet-network-ppp',   'packet-network-slip',   'stu-
       ...
       --set-bands=[BAND1|BAND2|...]
              ...
              ported bands and listing them all here would  be  quite  exten‐
              sive. For details, see the MMModemBand documentation.
       ...
       --set-preferred-mode=MODE
              ...
              option. Possible MODE arguments are detailed at  the  beginning
              of this section.
...
3GPP OPTIONS
       ...
       --3gpp-ussd-initiate=COMMAND
              Request the given modem to initiate a USSD  session  with  COM‐
              MAND.
...
SIMPLE OPTIONS
       ...
       --simple-connect=['KEY1=VALUE1,KEY2=VALUE2,...']
              ...
              can  use the --create-bearer options, plus any of the following
              ones:
       ...
       --messaging-create-sms=['KEY1=VALUE1,...']
              ...
              lowing:
                ...
                'text' Message text, in UTF-8. When sending, if the  text  is
                       ...
                       message will be broken into  multiple  parts  or  mes‐
                       sages.  Note that text and data are never given at the
                       same time.
...
FIRMWARE OPTIONS
       ...
       --firmware-select=ID
              Select  a firmware image from those installed on a given modem.
              A list of available firmware  images  can  be  seen  using  the
...
SIM OPTIONS
       ...
       --change-pin=PIN
              ...
              This must be used WITH --pin to supply the old PIN number.
...
SMS OPTIONS
       ...
       --store
              This  option  will store the SMS in the default storage defined
              by the modem, which may be either modem-memory  or  SMS-memory.
              To know what the existing default storage is, see the --messag‐
              ing-status option.
...
APPLICATION OPTIONS
       ...
       --timeout=SECONDS
              ...
              this command. This option is useful when executing long running
              operations, like --3gpp-scan.

EXAMPLES
   Send the PIN to the SIM card
       You'll need first to know which the proper path/index is for  the  SIM
       in your modem:
           $ mmcli -m 0 | grep SIM
   ...
   Listing SMS messages
       ...
       setup:
           ...
           $>  sudo  mmcli  -s  0   --create-file-with-data=/path/to/the/out‐
       put/file
