...
OPTIONS
       --options file
              ...
              ‘gpg-agent.conf’ and expected in the ‘.gnupg’ directory directly below the home directory of the user.
       ...
       --debug-quick-random
              ...
              down to standard random quality.  It is only used for testing and shall not be used for any production quality keys.  This option is
       ...
       --log-file file
              Append  all  logging  output to file.  This is very helpful in seeing what the agent actually does.  If neither a log file nor a log
              file descriptor has been set on a Windows platform, the Registry entry HKCU\Software\GNU\GnuPG:DefaultLogFile, if set,  is  used  to
       ...
       --ignore-cache-for-signing
              ...
              to control this behaviour but this command line option takes precedence.
       ...
       --pinentry-program filename
              ...
              On a Windows platform the default is to use the first existing program from this  list:  ‘bin\pinentry.exe’,  ‘..\Gpg4win\bin\pinen‐
              try.exe’, ‘..\Gpg4win\pinentry.exe’, ‘..\GNU\GnuPG\pinentry.exe’, ‘..\GNU\bin\pinentry.exe’, ‘bin\pinentry-basic.exe’ where the file
       ...
       --extra-socket name
              ...
              to  the local gpg-agent and use its private keys.  This allows to decrypt or sign data on a remote machine without exposing the pri‐
              vate keys to the remote machine.
...
FILES
       ...
       private-keys-v1.d/
              ...
              Note that on larger installations, it is useful to put predefined files into the directory ‘/etc/skel/.gnupg2’ so that newly created
              users  start  up with a working configuration.  For existing users the a small helper script is provided to create these files (see:
              [addgnupghome]).
...
SIGNALS
       ...
       SIGHUP This signal flushes all cached passphrases and if the program has been started with a configuration file, the configuration file  is
              ...
              gram, pinentry-invisible-char, default-cache-ttl,  max-cache-ttl,  ignore-cache-for-signing,  no-allow-external-cache,  allow-emacs-
              pinentry,  no-allow-mark-trusted, disable-scdaemon, and disable-check-own-socket.  scdaemon-program is also supported but due to the
              current implementation, which calls the scdaemon only once, it is not of much use unless you manually kill the scdaemon.
