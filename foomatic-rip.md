...
DESCRIPTION
       ...
       · foomatic-rip  does  not  only  apply option settings supplied by the
         ...
         searches  the  entire  job  for embedded option settings (only Post‐
         Script jobs). Here not only settings which affect the whole job  are
         taken into account, but also settings in the page headers, which are
         only valid for the page where they were found, so applications which
         produce  PostScript  code with page-specific printer option settings
         are fully supported.
...
DIRECT, SPOOLER-LESS PRINTING
   Options
       -v        verbose mode for debugging.  WARNING:  This  will  create  a
                 ...
                 opens a security loophole and should not be used in  produc‐
                 tion.
       ...
       file      is      stored      as      /etc/direct/<printer>.ppd     or
...
CONFIGURATION FILE
       ...
       /etc/foomatic/filter.conf  is  read whenever foomatic-rip is executed.
       It allows one to configure the behavior  of  foomatic-rip  as  follows
