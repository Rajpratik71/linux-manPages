...
DESCRIPTION
       Creates  device  URIs for local and network connected printers for use
       with CUPS.
...
OPTIONS
       To specify the port on a multi-port JetDirect:
              -p<port> or --port=<port> (Valid  values  are  1*,  2,  and  3.
              *default)
...
NOTES
       1. Example using 'lsusb' to obtain USB bus ID and USB device ID (exam‐
       ple only, the values you obtain will differ) :
