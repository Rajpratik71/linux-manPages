...
DESCRIPTION
       apgbfm  is  used to manage Bloom filter that is used to restrict pass‐
       word generation in APG pasword generation software. Usage of the Bloom
       filter  allows  to  speed up password check for large dictionaries and
       ...
       In  simple  words,  apgbfm  generates n hash values for every word and
       sets corresponding bits in filter file to 1. To check the word  apgbfm
       generates  the  same  hash functions for that word and if all n corre‐
       sponding bits in filter file are set to 1 then it  suppose  that  word
...
OPTIONS
       ...
       -d dictfile
              ...
              if  you  want to reject word 'root', you should insert in dict‐
              file words: root, Root, RoOt, ... ,  ROOT.   To  indicate  that
