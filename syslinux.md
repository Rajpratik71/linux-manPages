...
DESCRIPTION
       ...
       prompt.  The  user  can then type a kernel file name followed by any kernel parameters. The SYSLINUX bootloader does not need to know about
       the kernel file in advance; all that is required is that it is a file located in the root directory on the disk.
...
OPTIONS
       ...
       -s, --stupid
              Install a "safe, slow and stupid" version of SYSLINUX. This version may work on some very buggy BIOSes on which SYSLINUX would  oth‐
       ...
       -r, --raid
              RAID mode.  If boot fails, tell the BIOS to boot the next device in the boot sequence (usually the next hard disk) instead of  stop‐
...
FILES
   Configuration file
       All  the  configurable defaults in SYSLINUX can be changed by putting a file called syslinux.cfg in the install directory of the boot disk.
       ...
       default kernel [ options ... ]
              ...
              If  no  DEFAULT  or UI statement is found, or the configuration file is missing entirely, SYSLINUX drops to the boot: prompt with an
              error message (if NOESCAPE is set, it stops with a "boot failed" message; this is also the case for PXELINUX  if  the  configuration
              file is not found.)
   ...
   Other operating systems
       ...
       Because  neither Linux kernels, nor boot sector images have reliable magic numbers, syslinux will look at the file extension. The following
   ...
   Novice protection
       ...
       The  compile  time  and date of a specific syslinux version can be obtained by the DOS command "type ldlinux.sys". This is also used as the
