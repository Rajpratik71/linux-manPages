...
OPTIONS
       ...
       -c, --change-name=partnum:name
              ...
              the  most part, Linux ignores the partition name, but it may be important in some OSes. GPT fdisk sets a default name based on the partition type code. If you want to set a name that includes
              a space, enclose it in quotation marks, as in sgdisk -c 1:"Sample Name" /dev/sdb. Note that the GPT name of a partition is distinct from the filesystem name, which is encoded in the  filesys-
       ...
       -n, --new=partnum:start:end
              ...
              of the same block for the end sector. A partnum value of 0 causes the program to use the first available partition number.
...
BUGS
       As of October 2013 (version 0.8.8), sgdisk should be considered beta software. Known bugs and limitations include:
...
Roderick W. Smith                                                                                   0.8.8                                                                                           SGDISK(8)
