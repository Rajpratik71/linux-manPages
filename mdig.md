...
DESCRIPTION
       ...
       The {@server} option is a mandatory global option. It is the name or
       IP address of the name server to query. (Unlike dig, this value is not
...
GLOBAL OPTIONS
       ...
       queried.  port# is the port number that mdig will send its queries
       ...
       +[no]multiline
           ...
           on a single line, to facilitate machine parsing of the mdig
           output.
...
LOCAL OPTIONS
       ...
       looked up using nibble format under the IP6.ARPA domain. To use the
       older RFC1886 method using the IP6.INT domain specify the -i option.
       ...
       +timeout=T
           ...
           5 seconds for UDP transport and 10 for TCP. An attempt to set T to
