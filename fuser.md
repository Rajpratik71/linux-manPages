...
DESCRIPTION
       ...
       behavior, use the -4 and -6 options.  The socket(s) can  be  specified
       by  the local and remote port, and the remote address.  All fields are
...
OPTIONS
       ...
       -k, --kill
              Kill  processes  accessing the file.  Unless changed with -SIG‐
              NAL, SIGKILL is sent.  An fuser process never kills itself, but
