...
SYNOPSIS
       ...
       bridge fdb { add | append | del | replace } LLADDR dev DEV { local | temp } [ self ] [ master ] [ router ] [ use ] [ dst IPADDR ] [ vni VNI ] [ port PORT ] [ via DEVICE ]
...
bridge link - bridge port
   ...
   bridge link set - set bridge specific attributes on a port
       ...
       guard on or guard off
              Controls whether STP BPUDs will be processed by the bridge port. By default, the flag is turned off allowed BPDU processing. Turning this flag on will cause the port to stop processing STP
              BPDUs.
...
NOTES
       ...
       displaying the forwarding table, entries for all bridges are displayed.  Add/delete/modify commands determine the underlying bridge device based on the bridge to which the corresponding ethernet
       device is attached.
