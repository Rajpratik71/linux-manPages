...
Description
       ...
       Mlabel displays the current volume label, if present. If new_label is not given, and if neither
       the  c nor the s options are set, it prompts the user for a new volume label.  To delete an ex‐
       isting volume label, press return at the prompt.

       The label is limited to 11 single-byte characters, e.g. Name1234567.

       Reasonable care is taken to create a valid MS-DOS volume label.  If an invalid label is  speci‐
       fied,  mlabel changes the label (and displays the new label if the verbose mode is set). Mlabel
...
mtools-4.0.23                                   10Dec18                                      mlabel(1)
