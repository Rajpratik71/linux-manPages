...
DESCRIPTION
       ...
       note  that  standard  input,  output,  and  error may still point to a
       device on the old root file system, keeping it busy. They  can  easily
       be  changed when invoking chroot (see below; note the absence of lead‐
       ing slashes to make it work whether pivot_root has changed the shell's
       root or not).
