...
DESCRIPTION
       ...
       bzip2(1), or lzop(1).  All options specified are passed directly to cmp(1) or diff(1).  If only
       one  file  is  specified, then the files compared are file1 (which must have a suffix of a sup‐
       ported compression format) and  file1  from  which  the  compression  format  suffix  has  been
