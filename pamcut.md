...
SYNOPSIS
       pamcut  [-left  leftcol] [-right rightcol] [-top toprow] [-bottom bot‐
       tomrow] [-width width] [-height height] [-pad] [-verbose] [ left right
...
DESCRIPTION
       ...
       height arguments.  left and top have the  same  effect  as  specifying
       them  as  the argument of a -left or -top option, respectively.  width
       and height have the same effect as specifying them as the argument  of
       a  -width  or  -height  option, respectively, where they are positive.
...
OPTIONS
       ...
       -pad   If the rectangle you specify is not entirely within  the  input
              image,  pamcut  fails  unless  you  also specify -pad.  In that
