...
DESCRIPTION
       ...
       the target system.  This same port number must be used in the host
       GDBs "target remote" command, which will be described shortly.  Note
       that if you chose a port number that conflicts with another service,
       ...
       at anything except 9600 baud.)  That is "gdb TARGET-PROG", or "gdb
       --baud BAUD TARGET-PROG".  After that, the only new command you need
       to know about is "target remote" (or "target extended-remote").  Its
...
OPTIONS
       ...
       ·   Multi-process mode -- debug more than one program/process:
           ...
           process being debugged exits, so you can debug several processes
           in the same session.
