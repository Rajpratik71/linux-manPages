...
OPTIONS
       ...
       --expect-reply=BOOL
           ...
           code. If this is set to "no", the method call will be issued but
           no response is expected, the tool terminates immediately, and thus
           no response can be shown, and no success or failure is returned
           via the exit code. To only suppress output of the reply message
       ...
       --allow-interactive-authorization=BOOL
           ...
           operation, if the security policy is configured for this. Defaults
           to "yes".
       ...
       --watch-bind=BOOL
           ...
           appear in the file system before connecting to it. Defaults to
           off. When enabled, the tool will watch the file system until the
...
PARAMETER FORMATTING
       ...
       followed by the entries shall be specified. For variants, the
       ...
       "Eins". The string "Two" is assigned the 32-bit unsigned integer 2.
       The string "Yes" is assigned a positive boolean.
