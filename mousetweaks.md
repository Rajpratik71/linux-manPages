...
OPTIONS
       ...
       --ssc  Trigger  the secondary mouse button when the primary mouse but‐
              ton is held down for a specified amount of time.
       ...
       -g, --geometry
              Size  and position of the click-type window. Expects a standard
              X Window System geometry string. ie. WIDTHxHEIGHT+X+Y
...
FILES
       mousetweaks.pid
              The  daemon  creates a lock file in the $XDG_RUNTIME_DIR direc‐
              tory to keep track of its state. On systems with no  user  run‐
              time directory, the $XDG_CACHE_HOME directory is used.
