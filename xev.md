...
OPTIONS
       ...
       -geometry geom
               This option specifies the size and/or location of the window, if a window is to be cre‐
               ated.
       ...
       -id windowid
               This option specifies that the window with the given id should be monitored, instead of
               creating a new window.

       -root   This option specifies that the root window should be monitored, instead of  creating  a
               new window.

       -s      This option specifies that save-unders should be enabled on the window. Save unders are
               similar to backing store, but they refer rather to the saving of pixels off-screen when
               the  current window obscures other windows. Save unders are only advisory, and are nor‐
               mally set for popup dialogs and other transient windows.
       ...
       -event event_mask
               ...
               select  multiple types of events.  When not specified, all events are selected.  Avail‐
               able event masks: keyboard mouse expose visibility structure substructure  focus  prop‐
               erty colormap owner_grab_button randr button
