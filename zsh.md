...
DESCRIPTION
       ...
       Zsh  has  command  line editing, builtin spelling correction, program‐
       mable command completion, shell functions (with autoloading),  a  his‐
       tory mechanism, and a host of other features.
...
THE ZSH USERGUIDE
       ...
       rent state at http://zsh.sourceforge.net/Guide/.  At the time of writ‐
       ing,  chapters  dealing  with startup files and their contents and the
...
STARTUP/SHUTDOWN FILES
       ...
       with a /).  If one of the options is unset at any  point,  any  subse‐
       quent  startup file(s) of the corresponding type will not be read.  It
       is also possible for a file in $ZDOTDIR to re-enable GLOBAL_RCS.  Both
       ...
       that  it  be  kept  as small as possible.  In particular, it is a good
       idea to put code that does not need to be run for every  single  shell
       behind  a test of the form `if [[ -o rcs ]]; then ...' so that it will
       not be executed when zsh is invoked with the `-f' option.
...
SEE ALSO
       ...
       builtins(1),  zshcalsys(1),  zshcompwid(1),   zshcompsys(1),   zshcom‐
       pctl(1),  zshcontrib(1), zshexpn(1), zshmisc(1), zshmodules(1), zshop‐
       tions(1),  zshparam(1),  zshroadmap(1),  zshtcpsys(1),  zshzftpsys(1),
       zshzle(1)
