...
DESCRIPTION
       ...
       Internally apt-ftparchive can make use of binary databases to cache
       the contents of a .deb file and it does not rely on any external
       programs aside from gzip(1). When doing a full generate it
       automatically performs file-change checks and builds the desired
       ...
       contents
           ...
           Directories are not written to the output. If multiple packages
           own the same file then each package is separated by a comma in the
           output.
       ...
       release
           ...
           tree. It recursively searches the given directory for uncompressed
           and compressed Packages, Sources, Contents, Components and icons
...
THE GENERATE CONFIGURATION
   ...
   Dir Section
       ...
       prepended certain relative paths defined in later sections to produce
   ...
   TreeDefault Section
       ...
       Contents
           ...
           the default) then apt-ftparchive will integrate those package
   ...
   Tree Section
       ...
       Typically this is a setting such as dists/stretch.

       All of the settings defined in the TreeDefault section can be used in
       a Tree section as well as three new variables.
       ...
       Architectures
           ...
           used to indicate that this tree has a source archive. The
           architecture 'all' signals that architecture specific files like
           Packages should not include information about architecture all
           packages in all files as they will be available in a dedicated
           file.
...
THE BINARY OVERRIDE FILE
       ...
       scanpackages(1). It contains four fields separated by spaces. The
       first field is the package name, the second is the priority to force
       that package to, the third is the section to force that package to and
...
THE EXTRA OVERRIDE FILE
       ...
       package, the second is the tag and the remainder of the line is the
       new value.
...
OPTIONS
       ...
       --md5, --sha1, --sha256, --sha512
           ...
           where Index can be Packages, Sources or Release and Checksum can
       ...
       APT::FTPArchive::LongDescription
           This configuration option defaults to "true" and should only be
           set to "false" if the Archive generated with apt-ftparchive(1)
           also provides Translation files. Note that the Translation-en
