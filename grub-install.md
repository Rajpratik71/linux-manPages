...
DESCRIPTION
       ...
       --no-nvram
              don't update the `boot-device'/`Boot*' NVRAM variables. This option is only available on
              EFI and IEEE1275 targets.
       ...
       --uefi-secure-boot
              install an image usable with UEFI Secure Boot.  This option is only available on EFI and
              if the grub-efi-amd64-signed package is installed.
       ...
       INSTALL_DEVICE must be system device filename.  grub-install copies GRUB images into boot/grub.
