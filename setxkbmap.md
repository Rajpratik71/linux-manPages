...
OPTIONS
       ...
       -layout name
               ...
               option  may  only be used once. Multiple layouts can be speci‐
       ...
       -variant name
               ...
               description. The -variant option may only be used once. Multi‐
               ple  variants  can  be specified as a comma-separated list and
               will be matched with the layouts specified with -layout.

       -verbose|-v [level]
               Specifies level of verbosity in output messages.  Valid levels
               range  from  0  (least  verbose)  to  10  (most verbose).  The
               default verbosity level is 5.  If no level is specified,  each
               -v or -verbose flag raises the level by 1.
