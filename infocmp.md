...
DESCRIPTION
   ...
   Comparison Options [-d] [-c] [-n]
       ...
       of  the terminals, the value returned depends on the type of the capa‐
       bility:
       ...
       -c   produces a list of each capability that is common between two  or
            more  entries.   Missing  capabilities are ignored.  Each item in
            the list shows “=” after the capability  name,  followed  by  the
            capability value.
       ...
       -n   produces  a  list of each capability that is in none of the given
            ...
            Normally  only  the conventional capabilities are shown.  Use the
            -x option to add the BSD-compatibility capabilities  (names  pre‐
            fixed with “OT”).
   ...
   Source Listing Options [-I] [-L] [-C] [-r]
       ...
       bytes.  infocmp trims away less essential parts to make  it  fit.   If
       you  are converting to one of the (rare) termcap implementations which
       accept an unlimited size of termcap,  you  may  want  to  add  the  -T
       option.  More often however, you must help the termcap implementation,
       ...
       characters from terminfo format.  For a stricter BSD-compatible trans‐
   ...
   Other Options
       ...
       -E   Dump the capabilities of the given terminal as tables, needed  in
            ...
            ity structure in the <term.h>).  This option is useful  for  pre‐
            paring  versions of the curses library hardwired for a given ter‐
            minal type.  The tables are all declared static,  and  are  named
       ...
       -q   This makes the output a little shorter:
            ...
            ·   Make  the comparison listing shorter by omitting subheadings,
                and using “-”  for  absent  capabilities,  “@”  for  canceled
                rather than “NULL”.
       ...
       -v n prints out tracing information on standard error as  the  program
            ...
            ncurses  is built without tracing support, the optional parameter
            is ignored.
...
EXTENSIONS
       ...
       SVr4 infocmp does not distinguish between absent and  cancelled  capa‐
       bilities.   Also,  it  shows  missing  integer capabilities as -1 (the
       internal value used to represent missing integers).  This  implementa‐
       tion shows those as “NULL”, for consistency with missing strings.
