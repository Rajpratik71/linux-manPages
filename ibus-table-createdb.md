...
SYNOPSIS
       ibus-table-createdb [  [ -n name ]  [ --name name ]  ]  [  [ -s source ]  [ --source source ]  ]  [  [ -e extra ]  [ --extra extra ]  ]  [  [ -p pinyin ]  [ --pinyin pinyin ]  ]  [ -o ]  [ --no-cre‐
       ate-index ]  [ -i ]  [ --create-index-only ]  [ -d ]  [ --debug ]
...
OPTIONS
       ...
       -n     database-file specifies the file name for the binary database for the IME. The default is ''. If the file name of the database is not specified, the file name of the source  file  before  the
       ...
       -i     Only create an index for an existing database. Specifying the file name of the binary database with the -n or --name option is required when this option is used.
