...
SYNOPSIS
       ...
       The  see,  edit,  compose  and  print  versions  are just aliases that
       default to the view, edit, compose, and print actions (respectively).
...
DESCRIPTION
       ...
       with  those in the mime.types files.  If no mime-type is found, a last
       attempt will be done by running the file command,  if  available.   If
       the  encoding  is  omitted, it will also be determined from the file's
...
SECURITY
       ...
       Perl  regular  expresssion "[^[:alnum:],.:/@%^+=_-]", in order to pro‐
       ...
       file is opened using its absolute path to  prevent  the  injection  of
