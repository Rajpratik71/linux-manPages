...
DESCRIPTION
       ...
       modules and other files, except for the optional configuration files
       in the /etc/modprobe.d directory (see modprobe.d(5)).  modprobe will
       also use module options specified on the kernel command line in the
...
OPTIONS
       ...
       -b, --use-blacklist
           ...
           configuration files (if any) to module names as well. It is
       ...
       --first-time
           ...
           a module which is already present or to remove a module which
           isn't present. This is ideal for simple scripts; however, more
       ...
       --force-modversion
           ...
           kernel complains that the module disagrees about a version of some
       ...
       --show-depends
           ...
           distributions to determine which modules to include when
           generating initrd/initramfs images.  Install commands which apply
           are shown prefixed by "install". It does not run any of the
           install commands. Note that modinfo(8) can be used to extract
           dependencies of a module from the module itself, but knows nothing
