...
DESCRIPTION
       Download firmware to a device that requires downloaded firmware to function. (Note: Most print‐
       ers do not require the use of this utility).
...
OPTIONS
       ...
       Use USB IDs to specify printer:
              -s bbb:ddd, where bbb is the USB bus ID and ddd is the USB device ID. The  ':'  and  all
              leading zeroes must be present.
