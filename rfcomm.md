...
OPTIONS
       ...
       -i <hciX> | <bdaddr>
              The command is applied to device hciX, which must be the name or the address of an installed Bluetooth device. If not specified, the
...
COMMANDS
       ...
       connect <dev> [bdaddr] [channel]
              ...
              nel number 1. This command can be terminated with the key sequence CTRL-C.
       ...
       bind <dev> [bdaddr] [channel]
              This binds the RFCOMM device to a remote Bluetooth device. The command does not establish a connection to the remote device, it only
