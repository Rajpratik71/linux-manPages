...
DESCRIPTION
       ...
       When the user exits from the single-user shell, or presses control-D at the prompt, the  system
...
OPTIONS
       -e, --force
              ...
              Only use the -e option if you are sure the console is physically protected against unau‐
              thorized access.
...
ENVIRONMENT VARIABLES
       sulogin looks for the environment variable SUSHELL or sushell to determine what shell to start.
       If  the  environment variable is not set, it will try to execute root's shell from /etc/passwd.
       If that fails, it will fall back to /bin/sh.

AUTHOR
       sulogin was written by Miquel van Smoorenburg for sysvinit and later ported  to  util-linux  by
       Dave Reisner and Karel Zak.

AVAILABILITY
       The  sulogin  command  is  part  of  the  util-linux package and is available from Linux Kernel
