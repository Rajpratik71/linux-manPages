...
DESCRIPTION
       ...
       The filefrag program initially attempts to get the extent information using FIEMAP ioctl  which
       is more efficient and faster.  If FIEMAP is not supported then filefrag will fall back to using
       FIBMAP.
...
OPTIONS
       ...
       -bblocksize
              Use  blocksize in bytes for output instead of the filesystem blocksize.  For compatibil‐
              ity with earlier versions of filefrag, if blocksize is unspecified it defaults  to  1024
              bytes.
