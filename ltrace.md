...
SYNOPSIS
       ...
       [-n|--indent nr] [-o|--output filename] [-D|--debug  mask]  [-u  user‐
...
OPTIONS
       ...
       -D, --debug mask
              ...
              internal  meaning  that's not really well defined at all.  mask
       ...
       -F filename
              ...
              those two default files.  See ltrace.conf(5) for details on the
       ...
       -l, --library library_pattern
              ...
              directed to the selected libraries, there's no actual guarantee
              that   the  call  won't  be  directed  elsewhere  due  to  e.g.
              LD_PRELOAD or simply dependency ordering.  If you want to  make
       ...
       -n, --indent nr
              Indent  trace  output by nr spaces for each level of call nest‐
              ing. Using this option makes  the  program  flow  visualization
              easy  to  follow.   This  indents uselessly also functions that
              never return, such as service functions for throwing exceptions
              in the C++ runtime.
       ...
       -p pid Attach to the process with the process ID pid and  begin  trac‐
              ing.   This  option can be used together with passing a command
              to execute.  It is possible to attach to several  processes  by
...
FILTER EXPRESSIONS
       ...
       expressions  as well (see below).  The glob syntax supports meta-char‐
       acters * and ? and character classes, similarly  to  what  basic  bash
       globs  support.   ^  and $ are recognized to mean, respectively, start
       and end of given name.
       ...
       two *'s as appropriate.  The exception is if the pattern is  not  men‐
       tioned at all, in which case it's as if the corresponding pattern were
       *.  (So malloc is really malloc@* and @libc.* is really *@libc.*.)
       ...
       another rule @ in front of it, which has the effect of tracing comple‐
       ment of given rule.
...
BUGS
       ...
       (ltrace-devel@lists.alioth.debian.org), or use the  reportbug(1)  pro‐
