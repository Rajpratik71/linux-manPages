...
DESCRIPTION
       ...
       the  input  file.   See  http://www.ijg.org  for  information  on  the
       library.
       ...
       jpegtopnm  generates output with either one byte or two bytes per sam‐
       ple depending on whether the JFIF input has either 8 bits or  12  bits
       per sample.  You can use pnmdepth to reduce a two-byte-per-sample file
       ...
       To know what color a pixel is, one would have to know  the  properties
       of the inks to which the color space refers.  jpegtopnm interprets the
...
OPTIONS
       ...
       -maxmemory N
              ...
              bytes  if "M" is suffixed to the number.  For example, -maxmem‐
              ory 4m selects 4000000 bytes.  If jpegtopnm needs  more  space,
       ...
       -exif=filespec
              Extract  the  contents  of the EXIF header from the input image
              and write it to the file filespec.  filespec = - means write it
              to  Standard  Output.   In this case, jpegtopnm does not output
              ...
              You can use this file as input to ppmtojpeg to insert an  iden‐
              tical EXIF header into a new JFIF image.
       ...
       -comments
              Print  any  comments  in  the  input file to the Standard Error
              file.
...
HINTS
       ...
       deal with the NetPBM formats.  Also, the command syntax  of  jpegtopnm
