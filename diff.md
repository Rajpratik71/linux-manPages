...
DESCRIPTION
       ...
       --palette=PALETTE
              the  colors to use when --color is active; PALETTE is a colon-separated list of terminfo
              capabilities
       ...
       --to-file  is given, there are no restrictions on FILE(s).  If a FILE is '-', read standard in‐
       put.  Exit status is 0 if inputs are the same, 1 if different, 2 if trouble.
...
SEE ALSO
       ...
       The full documentation for GNU is maintained as a Texinfo manual.  If the info and GNU programs
