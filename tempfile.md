...
DESCRIPTION
       tempfile creates a temporary file in a safe manner.  It uses mkstemps(3) to choose the name and
       opens it with O_RDWR | O_CREAT | O_EXCL.  The filename is printed on standard output.
...
OPTIONS
       ...
       -n, --name FILE
              Use FILE for the name instead of tempnam(3).  The options -d, -p, and -s are ignored  if
