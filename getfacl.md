...
DESCRIPTION
       ...
       Line 4 indicates the setuid (s), setgid (s), and sticky (t) bits: either the letter  represent‐
       ing the bit, or else a dash (-). This line is included if any of those bits is set and left out
       otherwise, so it will not be shown for most files. (See  CONFORMANCE  TO  POSIX  1003.1e  DRAFT
       STANDARD 17 below.)
       ...
       all groups and to named users. (The file owner and others permissions are not affected  by  the
       effective rights mask; all other entries are.)  Lines 11--15 display the default ACL associated
       with this directory. Directories may have a default ACL. Regular files  never  have  a  default
       ACL.
...
OPTIONS
       ...
       -t, --tabular
           ...
           The  entry  tag  names for the ACL_USER_OBJ and ACL_GROUP_OBJ entries are also displayed in
...
CONFORMANCE TO POSIX 1003.1e DRAFT STANDARD 17
       ...
       only  printed if the -d option is given. If no command line parameter is given, getfacl behaves
       as if it was invoked as ``getfacl -''.  No flags comments indicating the  setuid,  setgid,  and
