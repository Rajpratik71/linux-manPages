...
DESCRIPTION
       ...
       Note  that  some filesystems and some partition tables store more magic strings on the device.  The wipefs command lists only the first offset where a magic string has been detected.  The device
       is not scanned for additional magic strings for the same filesystem.  It is possible that after a wipefs -o offset the same filesystem or partition table will still be visible because of another
...
OPTIONS
       ...
       -o, --offset offset
              Specify  the location (in bytes) of the signature which should be erased from the device.  The offset number may include a "0x" prefix; then the number will be interpreted as a hex value.
       ...
       -t, --types list
              Limit the set of printed or erased signatures.  More than one type may be specified in a comma-separated list.  The list or individual types can be prefixed with 'no' to specify the types
