...
DESCRIPTION
       ...
       user.conf and the files in user.conf.d directories. See systemd-
...
OPTIONS
       ...
       --system, --user
           ...
           debugging. Note that it is not supported booting and maintaining a
           full system with systemd running in --system mode, but PID not 1.
           In practice, passing --system explicitly is only useful in
       ...
       --crash-vt=VT
           ...
           attempted. This switch has no effect when running as user
       ...
       --show-status=
           ...
           line setting systemd.show_status= (see below) and the
       ...
       --log-location=
           ...
           debugging purposes. Argument is a boolean value. If the argument
       ...
       --service-watchdogs=
           ...
           emergency actions. This setting may also be specified during boot,
           on the kernel command line via the systemd.service_watchdogs=
...
CONCEPTS
       ...
       or programmatically at runtime. Units may be "active" (meaning
       ...
       (process returned error code on exit, or crashed, an operation timed
       out, or after too many restarts). If this state is entered, the cause
       will be logged, for later reference. Note that the various unit types
       ...
       in via dependencies. Usually, the unit name is just an alias (symlink)
       for either graphical.target (for fully-featured boots into the UI) or
       ...
       established Unix functionality such as /etc/fstab or the utmp database
       are supported.
...
DIRECTORIES
       System unit directories
           ...
           these directories only with the enable and disable commands of the
       ...
       SysV runlevel link farm directory
           ...
           account when figuring out whether a service shall be enabled. Note
           that a service unit with a native unit configuration file cannot
...
ENVIRONMENT
       ...
       $SYSTEMD_COLORS
           ...
           decision that systemd makes based on $TERM and what the console is
           connected to.
...
KERNEL COMMAND LINE
       ...
       systemd.unit=, rd.systemd.unit=
           ...
           initial RAM disk (initrd), while the one that is not prefixed only
           in the main system.

       systemd.dump_core
           Takes a boolean argument or enables the option if specified
           without an argument. If enabled, the systemd manager (PID 1) dumps
       ...
       systemd.crash_shell
           Takes a boolean argument or enables the option if specified
           without an argument. If enabled, the system manager (PID 1) spawns
           a shell when it crashes, after a 10s delay. Otherwise, no shell is
           spawned. Defaults to disabled, for security reasons, as the shell
       ...
       systemd.confirm_spawn
           ...
           the confirmation messages should be emitted. Can be also specified
           without an argument, with the same effect as a positive boolean.
           If enabled, the system manager (PID 1) asks for confirmation when
       ...
       systemd.show_status
           ...
           boot. Defaults to enabled, unless quiet is passed as kernel
           command line option, in which case it defaults to auto. If
           specified overrides the system manager configuration file option
           ShowStatus=, see systemd-system.conf(5). However, the process
           command line option --show-status= takes precedence over both this
       ...
       systemd.setenv=
           ...
           set default environment variables to add to forked child
       ...
       systemd.legacy_systemd_cgroup_controller
           ...
           previous option). When specified without an argument or with a
           true argument, disables the use of "hybrid" cgroup hierarchy (i.e.
           a cgroups-v2 tree used for systemd, and legacy cgroup
