...
DESCRIPTION
       ...
       for DNSKEY, DS and DLV records to establish a chain of trust for
       DNSSEC validation. It does not perform iterative resolution, but
       simulates the behavior of a name server configured for DNSSEC
...
SIMPLE USAGE
       ...
       server
           ...
           usable addresses are found, delv will send queries to the
...
OPTIONS
       -a anchor-file
           ...
           alternate key name can be specified using the +root=NAME options.
           DNSSEC Lookaside Validation can also be turned on by using the
           ...
           it will be necessary to update /etc/bind.keys to use DNSSEC
           validation in delv.
       ...
       -i
           ...
           is necessary to examine invalid data to debug a DNSSEC problem,
           use dig +cd.)
       ...
       -x addr
           ...
           provide the name or type arguments.  delv automatically performs a
           lookup for a name like 11.12.13.10.in-addr.arpa and sets the query
           type to PTR. IPv6 addresses are looked up using nibble format
...
QUERY OPTIONS
       ...
       +[no]rtrace
           ...
           1 using the -d option will product the same output (but will
       ...
       +[no]split[=W]
           ...
           into chunks of W characters (where W is rounded up to the nearest
       ...
       +[no]multiline
           ...
           verbose multi-line format with human-readable comments. The
