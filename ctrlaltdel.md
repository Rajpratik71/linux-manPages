...
DESCRIPTION
       Based  on  examination  of  the linux/kernel/reboot.c code, it is clear that there are two sup‐
       ported functions that the Ctrl-Alt-Del sequence can perform.

       hard   Immediately reboot the computer without calling sync(2) and without any  other  prepara‐
              tion.  This is the default.

       soft   Make  the  kernel send the SIGINT (interrupt) signal to the init process (this is always
...
AVAILABILITY
       The ctrlaltdel command is part of the util-linux package and is  available  from  Linux  Kernel
