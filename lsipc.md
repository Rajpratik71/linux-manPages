...
OPTIONS
       -i, --id id
              Show  full details on just the one resource element identified by id.  This option needs to be combined with one of the three resource options: -m, -q or -s.  The default --id output for‐
              mat is possible to override by the --list --raw --json or --export options.

       -g, --global
              Show system-wide usage and limits for a single IPC type.  This option may be combined with one of the three resource options: -m, -q or -s.  The default is to show information  about  all
              resources.
