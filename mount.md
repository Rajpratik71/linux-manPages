...
DESCRIPTION
       ...
       See section "Non-superuser mounts" below for more details.  The previ‐
       ous contents (if any) and owner and mode of dir become invisible,  and
       as long as this filesystem remains mounted, the pathname dir refers to
   ...
   Indicating the device and filesystem
       ...
       is also possible to indicate a block special device using its filesys‐
       tem  label or UUID (see the -L and -U options below), or its partition
       label or UUID.  Partition identifiers are supported  for  example  for
   ...
   The files /etc/fstab, /etc/mtab and /proc/mounts
       ...
       it  a  symlink  to  /proc/mounts  instead, because a regular mtab file
       maintained in userspace cannot reliably work with namespaces, contain‐
       ers and other advanced Linux features.
   ...
   Non-superuser mounts
       ...
       mountpoint  to specify filesystem, otherwise mount may fail. For exam‐
       ple it's bad idea to use NFS or CIFS source on command line.
       ...
       option is similar, with the restriction that the user must  be  member
   ...
   Bind mounts
       ...
       It's also possible to change nosuid, nodev, noexec,  noatime,  nodira‐
       time  and  relatime  VFS entry flags by "remount,bind" operation. It's
       ...
       impossible  to  re-define  mount  options without interaction with the
       bind semantic. This mount(8) behavior does not affect situations  when
   ...
   Shared subtree operations
       ...
       The  detailed  semantics  are  documented  in   Documentation/filesys‐
       ...
       requested.   All necessary information has to be specified on the com‐
       mand line.
       ...
       ble  to  specify  the  propagation  flags in fstab(5) as mount options
       (private, slave, shared, unbindable, rprivate, rslave,  rshared,  run‐
       bindable).
...
COMMAND-LINE OPTIONS
       ...
       mined by first extracting the mount options for  the  filesystem  from
       ...
       -f, --fake
              ...
              /etc/mtab and fails when the record already exists (with a reg‐
              ular non-fake mount, this check is done by the kernel).
       ...
       -l, --show-labels
              ...
              work.   One  can  set such a label for ext2, ext3 or ext4 using
       ...
       -O, --test-opts opts
              ...
              In this regard it is like the -t option except that -O is  use‐
              less without -a.  For example, the command:
       ...
       -t, --types fstype
              ...
              depend  on  the  running  kernel.   See  /proc/filesystems  and
              ...
              a  single  *,  mount  will  read  /proc/filesystems afterwards.
              ...
              order  (e.g.,  to try vfat before msdos or ext3 before ext2) or
              ...
              The nfs, nfs4, cifs, smbfs, and ncpfs filesystems have a  sepa‐
              rate  mount program.  In order to make it possible to treat all
              types  in  a  uniform  way,  mount  will  execute  the  program
              /sbin/mount.type  (if  that exists) when called with type type.
              Since different versions of the smbmount program have different
...
FILESYSTEM-INDEPENDENT MOUNT OPTIONS
       ...
       noatime
              ...
              This works for all inode types (directories too), so it implies
              nodiratime.
       ...
       rootcontext=context
              ...
              under SELinux, such as an  ext3  formatted  disk  from  a  non-
              SELinux  workstation.  You can also use context= on filesystems
              you do not trust, such as a floppy.  It also helps in  compati‐
              bility  with  xattr-supporting  filesystems  on earlier 2.4.<x>
              kernel versions.  Even where xattrs are supported, you can save
              time  not  having  to  label every file by assigning the entire
              ...
              It represents the entire filesystem for certain kinds  of  per‐
              mission  checks,  such as during mount or file creation.  Indi‐
              vidual file labels are still obtained from the  xattrs  on  the
              files  themselves.  The context option actually sets the aggre‐
              gate context that fscontext provides, in addition to  supplying
       ...
       lazytime
              Only update times (atime, mtime, ctime) on the  in-memory  ver‐
              sion of the file inode.
       ...
       remount
              ...
              After  this  call  all old mount options are replaced and arbi‐
              trary stuff from fstab (or mtab) is ignored, except  the  loop=
       ...
       user   Allow  an  ordinary  user to mount the filesystem.  The name of
              ...
              mtab) so that this same user can unmount the filesystem  again.
       ...
       x-*    The  same  as  X-*  options, but stored permanently in the user
              ...
              tools and there is no guarantee that the options will be always
              available (for example after  a  move  mount  operation  or  in
              unshared namespace).
              ...
              was  the  same  as  have X-* now), but due to growing number of
              use-cases (in initrd, systemd etc.) the functionality have been
...
FILESYSTEM-SPECIFIC MOUNT OPTIONS
   ...
   Mount options for btrfs
       ...
       max_inline=bytes
              ...
              with some space unavailable due to leaf headers.  For a 4k sec‐
              torsize, max inline data is ~3900 bytes.
       ...
       subvolrootid=objectid  (deprecated)
              ...
              volume.  This allows mounting of subvolumes which  are  not  in
              the root of the mounted filesystem.  You can use "btrfs subvol‐
              ume show " to see the object ID for a subvolume.

       thread_pool=number
              The number of worker threads to allocate.  The  default  number
              is equal to the number of CPUs + 2, or 8, whichever is smaller.
   ...
   Mount options for debugfs
       ...
       on /sys/kernel/debug.  As of kernel version 3.4, debugfs has the  fol‐
       lowing options:
   ...
   Mount options for fat
       ...
       dos1xfloppy
              ...
              ters  match  defaults  assumed by DOS 1.x for 160 kiB, 180 kiB,
              320 kiB, and 360 kiB floppies and floppy images.

       errors={panic|continue|remount-ro}
              Specify FAT behavior on critical errors: panic, continue  with‐
              out  doing anything, or remount the partition in read-only mode
              (default behavior).
       ...
       nfs={stale_rw|nostale_ro}
              ...
              evicted  from  the  inode cache.  However, it means that opera‐
              tions such as rename, create and unlink could cause  file  han‐
              dles  that previously pointed at one file to point at a differ‐
              ent file, potentially causing data corruption.  For  this  rea‐
              son, this option also mounts the filesystem readonly.
       ...
       tz=UTC This option disables the conversion of timestamps between local
              ...
              (like  digital  cameras)  that are set to UTC in order to avoid
       ...
       time_offset=minutes
              ...
              in presence of DST - time stamps in  a  different  DST  setting
   ...
   Mount options for hfs
       ...
       dir_umask=n, file_umask=n, umask=n
              ...
              all  files  and directories.  Defaults to the umask of the cur‐
              rent process.
   ...
   Mount options for nfs and nfs4
       ...
       nfs_mount_data)  to  the  mount  system  call.   This argument is con‐
       structed by mount.nfs(8) and the current version of mount (2.13)  does
       not know anything about nfs and nfs4.
   ...
   Mount options for ntfs
       ...
       posix=[0|1]
              ...
              hard  links  instead of being suppressed.  This option is obso‐
              lete.
   ...
   Mount options for overlay
       ...
       must  provide a valid d_type in readdir responses, so NFS is not suit‐
       able.

       A read-only overlay of two read-only filesystems may use any  filesys‐
       tem  type.   The  options  lowerdir  and  upperdir are combined into a
   ...
   Mount options for reiserfs
       ...
       nolog  Disable  journaling.   This  will  provide  slight  performance
              ...
              save for actual writes into its journaling  area.   Implementa‐
   ...
   Mount options for tmpfs
       ...
       mpol=[default|prefer:Node|bind:NodeList|interleave|inter‐
       leave:NodeList]
              ...
              The NodeList format is a comma-separated list of  decimal  num‐
              bers  and  ranges,  a  range being two "hyphen-minus"-separated
              ...
              system relies on that tmpfs being mounted,  but  from  time  to
              time  runs  a  kernel  built without NUMA capability (perhaps a
              ...
              MountPoint,  by  'mount  -o remount,mpol=Policy:NodeList Mount‐
              Point'.
   ...
   Mount options for ufs
       ufstype=value
              ...
              the type of ufs automatically.  That's why the user must  spec‐
              ify the type of ufs by mount option.  Possible values are:
   ...
   Mount options for vfat
       ...
       uni_xlate
              ...
              '?'  is used when no translation is possible.  The escape char‐
              acter is ':' because  it  is  otherwise  invalid  on  the  vfat
              filesystem.  The escape sequence that gets used, where u is the
...
THE LOOP DEVICE
       One further possible type is a mount via the loop device.   For  exam‐
       ple, the command
       ...
       for  some loop device with the same offset and sizelimit. This is nec‐
       essary to avoid a filesystem corruption.
...
EXTERNAL HELPERS
       ...
       comment,  x-*,  loop,  offset  and  sizelimit  to  the  mount.<suffix>
...
BUGS
       ...
       depends on the kernel and others settings (e.g. on a remote NFS server
       -- in certain cases the mount command may report  unreliable  informa‐
       tion  about  an NFS mount point and the /proc/mounts file usually con‐
       tains more reliable information.)  This is another reason  to  replace
       the mtab file with a symlink to the /proc/mounts file.
       ...
       size of the block device has been configured as requested.  This situ‐
       ation  can  be  worked  around  by  using the losetup command manually
