...
DESCRIPTION
       ...
       although for check operations, the table is locked with a READ lock only. Table maintenance
       operations can be time-consuming, particularly for large tables. If you use the --databases or
       --all-databases option to process all tables in one or more databases, an invocation of
       mysqlcheck might take a long time. (This is also true for mysql_upgrade because that program
       invokes mysqlcheck to check all tables and repair them if necessary.)
       ...
       myisamchk should be used when it is not. The benefit of using mysqlcheck is that you do not
       ...
       The MyISAM storage engine supports all four maintenance operations, so mysqlcheck can be used
       to perform any of them on MyISAM tables. Other storage engines do not necessarily support all
       operations. In such cases, an error message is displayed. For example, if test.t is a MEMORY
       ...
       repair strategies. This will be the case, for example, for InnoDB tables, which can be checked
       ...
       or make a symbolic link to mysqlcheck named mysqlrepair. If you invoke mysqlrepair, it repairs
       tables.
       ...
       the [mysqlcheck] and [client] option file groups.  The -c, -r, -a and -o options are exclusive
       to each other.
       ...
       ·   --check-upgrade, -g
           ...
           the current version of the server. This option automatically enables the --fix-db-names
       ...
       ·   --databases, -B
           ...
           argument on the command line as a database name and following names as table names. With
       ...
       ·   --extended, -e
           ...
           If you are using this option to check tables, it ensures that they are 100% consistent but
           takes a long time.

           If you are using this option to repair tables, it will force using the old, slow, repair
           with keycache method, instead of the much faster repair by sorting.
       ...
       ·   --fix-table-names
           ...
           Convert table names (including views) to the format used since MySQL 5.1. Only table names
           that contain special characters are affected.

       ·   --flush,

           Flush each table after check. This is useful if you don't want to have the checked tables
           take up space in the caches after the check.
       ...
       ·   --password[=password], -p[password]
           ...
           value following the --password or -p option on the command line, mysqlcheck prompts for
           one.
       ...
       ·   --process-tables
           ...
           Perform the requested operation on tables. Defaults to on; use --skip-process-tables to
           disable.
       ...
       ·   --quick, -q
           ...
           If you are using this option to check tables, it prevents the check from scanning the rows
           to check for incorrect links. This is the fastest check method.
       ...
       ·   --use-frm
           ...
           For repair operations on MyISAM tables, get the table structure from the .frm file so that
           the table can be repaired even if the .MYI header is corrupted.
       ...
       ·   --verbose, -v
           ...
           Verbose mode. Print information about the various stages of program operation.  Using one
       ...
       ·   --write-binlog
           ...
           the --skip-write-binlog when these statements should not be sent to replication slaves or
...
MariaDB 10.5                                 27 June 2019                               MYSQLCHECK(1)
