...
DESCRIPTION
       tpm2_rsaencrypt(1)  -  Performs  RSA  encryption on the contents of file data using the indicated padding scheme according to IETF RFC 3447
       (PKCS#1).  Input defaults to STDIN if not specified.
...
OPTIONS
       • -c, --key-context=OBJECT:
         ...
         Context object pointing to the the public portion of RSA key to use for encryption.

       • -o, --output=FILE:

         Optional output file path to record the encrypted data to.  The default is to print the binary encrypted data to stdout.
       ...
       • -l, --label=FILE or STRING:
         ...
         Optional, set the label data.  Can either be a string or file path.  The TPM requires the last byte of the label to be zero, this is han‐
         dled internally to the tool.  No other embedded 0 bytes can exist or the TPM will truncate your label.

       • ARGUMENT the command line argument specifies the path of the file with data to be encrypted.
