tpm2_nvcertify(1)                                             General Commands Manual                                            tpm2_nvcertify(1)
...
DESCRIPTION
       tpm2_nvcertify(1)  -  Provides  attestation of the contents of an NV index.  NOTE: As part of the attestation output, the NV index contents
       are revealed.
...
OPTIONS
       ...
       • -c, --nvauthobj-context=OBJECT:
         ...
         The object that is the authorization handle for the NV object.  It is either the NV index handle itself or the platform/ owner  hierarchy
       ...
       • --size=NATURAL_NUMBER:
         ...
         Specifies  the  size  of data to be read in bytes, starting from 0 if offset is not specified.  If not specified, the size of the data as
