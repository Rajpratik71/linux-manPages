...
TRYING OUT A DIFFERENT VERSION OF LIBVIRT
       ...
       The first command kills any session "libvirtd" process(es) that may be running on the machine.  The second command uses libvirt’s "run" script (in the top-level libvirt build directory) to set some
