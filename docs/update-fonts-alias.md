...
DESCRIPTION
       ...
       This enables multiple packages to provide aliases for fonts in the same directory.  No font package actually provides the fonts.alias file in the X font directory itself, so there is no  dan‐
       ger of overwriting one package's aliases with those of another.
...
BUGS
       See the Debian Bug Tracking System ⟨https://bugs.debian.org/xfonts-utils⟩.  If you wish to report a bug  in  update-fonts-alias,  please  see  /usr/share/doc/debian/bug-reporting.txt  or  the
       reportbug(1) command.
