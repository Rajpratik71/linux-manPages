tpm2_nvsetbits(1)                                             General Commands Manual                                            tpm2_nvsetbits(1)
...
DESCRIPTION
       tpm2_nvsetbits(1)  -  Bitwise  OR bits into a Non-Volatile (NV).  The NV index must be of type “bits” which is specified via the “nt” field
       when creating the NV space with tpm2_nvdefine(1).  The index can be specified as raw handle or an offset  value  to  the  NV  handle  range
       “TPM2_HR_NV_INDEX”.
...
OPTIONS
       ...
       • -i, --bits=BITS:
         ...
         Specifies the bit value as a number to bitwise OR into the current value of the NV index.
