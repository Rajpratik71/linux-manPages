...
OPTIONS
       --accept=accept-string
              Specifies  a  UNO-URL  connect-string to create a UNO acceptor through which other pro‐
              grams can connect to access the API. Note that API access allows execution of arbitrary
              commands.  The syntax of a UNO-URL connect-string is: uno:connection-type,params;proto‐
              col-name,params;ObjectName
       ...
       --headless
              ...
              This special mode can be used when the application is controlled  by  external  clients
              via the API.
       ...
       --norestore
              Disables  restart  and  file recovery after a system crash. It is possible that LO will
              try to restore a file it keeps crashing on, if that happens --norestore is the only way
              to start LO.
