...
DESCRIPTION
       tpm2_send(1) - Sends a TPM command to the TPM.  The command is read from a file as a binary stream and transmitted to the TPM using the TC‐
       TI specified by the caller.  The response received from the TPM is written to the output file.
