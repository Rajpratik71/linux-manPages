...
DESCRIPTION
   ...
   Overview
     ...
     interpretative language is common to both interactive and non-interac‐
     tive use (shell scripts).  That is, commands can be typed directly to
     the running shell or can be put into a file and the file can be executed
   ...
   Argument List Processing
     ...
     builtin (described later).
           ...
           -c               Read commands from the command_string operand
                            ...
                            erand and the positional parameters ($1, $2,
   ...
   Simple Commands
     ...
     ing actions:
           ...
           2.   The remaining words are expanded as described in the section
                ...
                ered the command name and the command is located.  The
   ...
   Redirections
     ...
     Following is a list of the possible redirections.  The [n] is an
     optional number, as in ‘3’ (not ‘[3]’), that refers to a file descrip‐
     tor.
   ...
   Pipelines
     ...
     it executes in the current shell -- but any effect it has on the envi‐
     ronment is wiped).
   ...
   Variables and Parameters
     ...
     numeric.  A parameter can also be denoted by a number or a special char‐
   ...
   Special Parameters
     ...
     *            Expands to the positional parameters, starting from one.
     ...
     - (Hyphen.)  Expands to the current option flags (the single-letter
                  ...
                  invocation, by the set builtin command, or implicitly by
                  the shell.
   ...
   Word Expansions
     ...
     expand to a single field.  It is only field splitting or pathname expan‐
     sion that can create multiple fields from a single word.  The single
   ...
   Shell Patterns
     ...
     character class.  A character class matches any of the characters
   ...
   Builtins
     ...
     getopts optstring var
            ...
            getopts will then unset OPTARG and write output to standard
            error.  By specifying a colon as the first character of optstring
     ...
     printf format [arguments ...]
            ...
            are converted and copied to the standard output, and format spec‐
            ifications, each of which causes printing of the next successive
            argument.

            The arguments after the first are treated as strings if the cor‐
            responding format is either b, c or s; otherwise it is evaluated
     ...
     [ expression ]
            ...
            n1 -ne n2     True if the integers n1 and n2 are not algebraical‐
                          ly equal.
...
ENVIRONMENT
     ...
     MAILCHECK  The frequency in seconds that the shell checks for the
                arrival of mail in the files specified by the MAILPATH or the
