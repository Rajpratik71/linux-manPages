...
OPTIONS
       ...
       -c <name>=<value>
           ...
           (like git -c foo.bar= ...) sets foo.bar to the empty string which
           git config --bool will convert to false.
...
ENVIRONMENT VARIABLES
   ...
   The Git Repository
       ...
       GIT_CEILING_DIRECTORIES
           ...
           environment. Normally, Git has to read the entries in this list
           and resolve any symlink that might be present in order to compare
           them with the current directory. However, if even this access is
           slow, you can add an empty entry to the list to tell Git that the
   ...
   other
       ...
       GIT_SSH, GIT_SSH_COMMAND
           ...
           passed to the configured command are determined by the ssh
       ...
       GIT_TRACE_PACKFILE
           ...
           displaying it on the terminal or mixing it with other trace
           output.
       ...
       GIT_REDACT_COOKIES
           ...
           header sent by the client is dumped, values of cookies whose key
           is in that list (case-sensitive) are redacted.
       ...
       GIT_PROTOCOL_FROM_USER
           ...
           programs which feed potentially-untrusted URLS to git commands.
       ...
       GIT_PRINT_SHA1_ELLIPSIS (deprecated)
           ...
           value. This affects indications of detached HEADs (git-
           checkout(1)) and the raw diff output (git-diff(1)). Printing an
           ellipsis in the cases mentioned is no longer considered adequate
           and support for it is likely to be removed in the foreseeable
           future (along with the variable).
...
DISCUSSION
       ...
       hold file data; trees, which point to blobs and other trees to build
       up directory hierarchies; and commits, which each reference a single
       ...
       recent commit (or "head") of a branch under development. SHA-1 names
       of tags of interest are stored under ref/tags/. A special ref named
       ...
       branch. The attributes (last modified time, size, etc.) are taken from
       the corresponding file in the working tree. Subsequent changes to the
...
REPORTING BUGS
       ...
       the Git Security mailing list <git-security@googlegroups.com[6]>.
