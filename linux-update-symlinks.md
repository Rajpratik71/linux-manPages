...
DESCRIPTION
       linux-update-symlinks  is intended to be called from the postinst and postrm maintainer scripts
       of Linux kernel packages.  The postinst script must pass the first argument install or  upgrade
       ...
       By default, this command maintains symlinks in the root directory.  This behaviour can be modi‐
       fied by settings in /etc/kernel-img.conf:
       ...
       The  symlinks for the primary default kernel version are named vmlinuz or vmlinux (depending on
       whether the architecture normally uses compressed kernel images) and initrd.img (if it uses  an
       ...
       The  top two entries on the list are the new primary and secondary default versions, and it up‐
       dates the symlinks accordingly.  In case there is only one entry, this is both the primary  and
       secondary  default  version.  If there are no entries, there are no default versions and it re‐
       moves the default symlinks.
...
AUTHOR
       linux-update-symlinks  and this manual page were written by Ben Hutchings as part of the Debian
       linux-base package.
