...
DESCRIPTION
       ...
       with  GNU troff.  If no files are given on the command line or a file‐
...
LANGUAGE OVERVIEW
       ...
       another table can be defined by using the request .T& before the final
       ...
       changed  by  the global option tbl(c), where c is an arbitrary charac‐
       ter.
...
USAGE
   ...
   Table format specification
       ...
       character describes each column of each line of the table.  Key  char‐
       acters  can be separated by spaces or tabs.  You may run format speci‐
       fications for multiple lines together on the same line  by  separating
       them with commas.
       ...
       n,N    Numerically justify item in the column: Units positions of num‐
              bers are aligned vertically.  If there  is  one  or  more  dots
              adjacent  to a digit, use the rightmost one for vertical align‐
              ment.  If there is no dot, use the rightmost digit for vertical
              alignment;  otherwise,  center  the  item  within  the  column.
              ...
              can happen if the table format is changed with .T& – center the
              widest  number  (of  the  data  entered  under  the N specifier
   ...
   Column specifiers
       ...
       m,M    This is a GNU tbl extension.  Either of these specifiers may be
              followed by a macro name (either one or two  characters  long),
              or  long  name in parentheses.  A one-letter macro name must be
              ...
              change the text block formatting, like text adjustment, hyphen‐
              ation,  size,  or  font.   The macro is called after other cell
              modifications like b, f or v are output.  Thus  the  macro  can
       ...
       not mutually exclusive with w); if specified multiple times for a par‐
       ticular column, the last entry takes effect: x unsets both  e  and  w,
   ...
   Text blocks
       ...
       and closed with ‘T}’.  The former must end a line, and the latter must
       ...
       cise, the line length used to process the  text  block  diversion)  is
...
Groff Version 1.22.3           10 February 2018                        TBL(1)
