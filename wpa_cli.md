...
OVERVIEW
       ...
       non-root user access (ctrl_interface GROUP= parameter in the  configu‐
       ration file). This makes it possible to run wpa_cli with a normal user
       account.
...
INTERACTIVE AUTHENTICATION PARAMETERS REQUEST
       ...
       word  and  otp  commands can be used regardless of whether the request
       was for PASSWORD or OTP. The main difference between  these  two  com‐
       mands  is  that  values  given with password are remembered as long as
       ...
       new value for every use.  This can be used to implement one-time-pass‐
       word lists and generic token card -based authentication.
...
COMMAND ARGUMENTS
       ...
       -G ping interval
              Set the interval (in seconds) at which wpa_cli pings  the  sup‐
              plicant.
